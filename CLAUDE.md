...
longer strands an authorized deployment. The workflow pipes the classifier
from the triggering commit into `python3 -I -` under `set -euo pipefail` (never
via a temp file, which a later line could redirect); the refreshed head cannot
...
1. **Fail-closed typed schema, explicit opaque islands** — wrapper/resource/nested keys unknown to this companion version are rejected with source file + YAML path before lossy re-serialization. Intentionally free-form plugin `config`, credential *entry values*, and per-item mesh values round-trip unchanged to the authoritative gateway validator. Consumer credential *map keys* are the closed Ferrum Edge built-in set (`KNOWN_CREDENTIAL_TYPES`); unknown keys fail closed before apply and before import publication.
   The one escape hatch is `FERRUM_ALLOW_UNKNOWN_FIELDS=true` (`config::LoadOptions`, threaded from `main` — never read from the process env inside the parse path): unknown **top-level** `spec` fields land in a `#[serde(flatten)]` `extra: BTreeMap` (`schema::PassthroughFields`) and flow through overlay merge → export → diff → apply verbatim, with one `Warning:` per file on **stderr** (stdout carries the exported YAML). Nested unknowns stay fatal in both modes — `serde_ignored` still sees them, because `flatten` only intercepts keys the struct did not claim. A pass-through key present only on the *live* side is not drift (`compare_fields` skips it); declaring a key in the repo is how the repo takes ownership of it. **`import` inverts this**: the value came from the gateway rather than the operator, and the credential broker only redacts leaves it models, so a non-empty `extra` on an importable resource is refused (`import::reject_import_passthrough_fields`) rather than written into the tree. `import --accept-unknown-field NAME` (repeatable) is the per-field acknowledgement, and it additionally requires `FERRUM_ALLOW_UNKNOWN_FIELDS=true` — otherwise the strict loader would reject the files import just wrote. `ImportPassthroughPolicy` carries both; `split_config` (the library entry point) is `strict()`. Acknowledged fields are named on stderr via `ImportResult::acknowledged_passthrough_notice`, and error text passes resource ids and field names through `diagnostic_metadata` because both come from an untrusted backup. Unknown Consumer credential map keys are likewise refused before publication (`import::reject_import_unknown_credential_types`), using `unknown_credential_type_message` and with no acknowledgement flag. Nested unknowns on import are refused too: `BackupSnapshot::from_value_with_strictness` decodes through `serde_ignored` and records each skipped nested field with its resource identity and `.spec…` path in `unmodeled_nested_fields`, and `import::reject_import_unmodeled_nested_fields` fails file and API import on any of them before a tree file or credential bundle is written. There is no acknowledgement flag, because the loader has no representation for a nested unknown; read-only live comparisons ignore the list. The list is also copied onto `BackupExtras::unmodeled_nested_fields`, and `apply::api_target::prepare_apply` blocks a namespace (`unmodeled_field_block`, per-namespace like the spec-owned conflict block — never per-row, which would break dependency ordering) when an affected row will actually be written: for incremental, `incremental_rewrite_keys` = Modify diffs + `pending_create_assertions` + (shared mode only) exactly the rows `adoption_candidates` returns (the adoption PUTs; a declared unledgered row that differs from live is never adopted, so it must not count); for full replace, every row of the `/restore` body. An unchanged declared row that needs no claim (always, in exclusive mode) must not block, or a newly serialized defaulted field would wedge every apply. The same block covers live-only unknown **top-level** fields: the live decode always keeps them in `extra` (regardless of `FERRUM_ALLOW_UNKNOWN_FIELDS`), a PUT/restore row built from the declaration would drop them, so `undeclared_live_top_level_fields` lists every declared row's live `extra` key its declaration lacks (as `.spec.<field>`) and the rewrite set filters them identically. We refuse rather than carry them through: silently merging live values into a repository write would make the gateway, not the repo, the source of those fields. A key the declaration names (possible only with `FERRUM_ALLOW_UNKNOWN_FIELDS=true`) is sent and diffed normally, so declaring it is the documented remedy besides upgrading. Preserved spec-owned rows in a `/restore` body are copied from live with `extra` intact, so only their nested fields can block. `prepare_apply` therefore takes the `OwnershipScope` (so does `preflight_api_apply`), and a supplied live view without its `BackupExtras` is refused rather than skipping the check. `preflight_api_apply` returns the per-namespace refusals (`BlockedNamespaces`, spec conflicts included); `cmd_apply` allocates credentials from `ResolveReport::without_namespaces` and journals creates only outside that set, and the interactive preview prints the set via `apply_blocked_namespaces` (same preparation, no `/health` probe). The later re-reads (post-plugin proxy snapshot, shared adoption confirmation, ambiguous-create verification) intentionally do not re-check either kind. Never suggest "stop declaring the row" as a remedy: in exclusive mode that deletes the live row. Both refusals render offenders through `http_client::describe_unmodeled_nested_fields`, capped at `MAX_LISTED_UNMODELED_NESTED_FIELDS` with an `…and N more` tail. Keep `config::schema` nested structs in step with each ferrum-edge pin bump so live rows do not trip either refusal.
...
- `FERRUM_ALLOW_UNKNOWN_FIELDS` (default `false`) — keep unknown top-level `spec` fields verbatim instead of rejecting them; nested unknowns stay fatal. For a gateway newer than this release. It does not lift apply's refusal to rewrite a row whose live copy carries a top-level field the repository does not declare; declaring that field under the flag does.
