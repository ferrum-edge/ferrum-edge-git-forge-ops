...
`import` refuses them before writing any tree file or migration bundle. The array
...
- `validator_namespace_tests.rs` exercises the shared command paths with a namespace-checking stub and the real Edge binary. Real-binary tests skip when `GITFORGEOPS_TEST_EDGE_BINARY` is absent; `rust-ci.yml` sets it after the trusted installer verifies the candidate checksum allowlist, using the same pin as `validate-pr.yml`.
