...
paths:
  ...
  - "src/policy/**"
  - "src/secrets/plugin_config.rs"
  - "src/config/schema.rs"
  - "tests/unit/{analysis,policy,secrets,schema}_tests.rs"
---

# Plugin catalog, policy, and secret rules
...
- Treat `PluginConfig.config` as secret-bearing. Import converts sensitive string leaves to exact
  broker placeholders; diff and review mask those leaves without hiding non-secret sibling drift.
- Known plugins use the schema-aware sensitive-path classifier. Unknown/custom plugins fail closed
  by brokering every string value.
- Never print resolved plugin secrets, literal consumer credentials, credential bundles, or
  validator diagnostics that may echo them.
...
Update the relevant flat unit modules and run the mandatory repository gate. Secret-classifier
changes also require focused `secrets::plugin_config::tests` coverage.
