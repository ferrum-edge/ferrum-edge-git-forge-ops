...
usage() {
  printf '%s\n' \
    ...
    '' \
    'Pinned to alibaba-token-plan/deepseek-v4-flash-0731. --model is accepted only' \
    'when it names that exact model; any other value is refused. The dated' \
    'snapshot is the only variant this skill dispatches.' \
    '' \
...
if [[ -n "$effort" ]]; then
  case "$effort" in
    medium|high|xhigh|max)
      printf '[deepseek-flash-agents] ignoring --effort %s: %s exposes no effort tiers\n' \
        "$effort" "$model" >&2
      ;;
    *)
...
# select a different DeepSeek variant or a rolling alias.
readonly PINNED_MODEL='alibaba-token-plan/deepseek-v4-flash-0731'
...
printf '[deepseek-flash-agents] dispatch model=%s worktree=%s bin=%s\n' \
