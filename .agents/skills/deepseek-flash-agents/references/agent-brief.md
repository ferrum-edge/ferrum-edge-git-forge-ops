# Ferrum Edge Git Forge Ops DeepSeek V4 Flash implementer brief

You are an opencode worker (model `alibaba-token-plan/deepseek-v4-flash-0731`) dispatched by an
orchestrator through the local opencode CLI harness. Implement or fix the scoped Ferrum Edge Git Forge Ops task
...
`deepseek-flash-agents`, `opencode-laguna-agents`, `grok-agents`, `sol-agents`, `luna-agents`, `opus-agents`,
