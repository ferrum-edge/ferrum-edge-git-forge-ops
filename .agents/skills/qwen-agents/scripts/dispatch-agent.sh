...
usage() {
  printf '%s\n' \
    ...
    '' \
    'Pinned to alibaba-token-plan/qwen3.8-max. --model is accepted only when' \
...
if [[ -n "$effort" ]]; then
  case "$effort" in
    medium|high|xhigh|max)
      printf '[qwen-agents] ignoring --effort %s: %s exposes no effort tiers\n' \
        "$effort" "$model" >&2
      ;;
    *)
...
readonly PINNED_MODEL='alibaba-token-plan/qwen3.8-max'
