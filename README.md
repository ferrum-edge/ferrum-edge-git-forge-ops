...
If you would rather accept the reassignment as-is, pass `--allow-credential-slot-remap` (global; works on `plan`, `apply`, `export --materialize` and `rotate`). It downgrades the refusal to the report it replaced — the hazard is still printed and still rendered in the PR comment, it just no longer stops the run. The ledger-backed deleted-Consumer and revived-slot checks run in `plan`, `review`, `apply` and `export --materialize` (which writes resolved values out); `rotate` resolves without the ledger because it replaces the slot's value.
