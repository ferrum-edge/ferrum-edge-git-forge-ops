...
- `allowed_backend_domains` skips a proxy's placeholder `backend_host` only when `upstream_id` exactly resolves to a declared upstream in the same namespace; empty, padded, unresolved, and cross-namespace references fall back to checking `backend_host`. The rule always checks static `targets[*].host` values and any proxy `dns_override`. Because service discovery can publish destinations that are not present in the static document, a discovery-backed upstream is reported as unverifiable and requires static targets or a reviewed override backed by equivalent runtime egress controls. `*.example.com` matches subdomains like `api.example.com` and `deep.api.example.com`; list `example.com` separately if the root domain is allowed too. IP literals, including `dns_override`, must be listed exactly; wildcard entries only apply to DNS names.
