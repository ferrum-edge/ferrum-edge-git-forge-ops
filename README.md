...
- `require_auth_plugin` evaluates each proxy's *effective* plugin list — scoped plugin configs merged over global ones of the same `plugin_name`, with disabled instances discarded. Omit `auth_plugin_names` to accept all eleven built-in authenticators: `spiffe_identity`, `mtls_auth`, `jwks_auth`, `oauth2_introspection`, `oidc_relying_party`, `jwt_auth`, `key_auth`, `ldap_auth`, `basic_auth`, `hmac_auth`, `soap_ws_security`. The security audit and breaking-change detection use the same allowlist to classify authenticators, even when the rule is disabled.
- `require_auth_plugin` and the security audit's "No auth plugin" warning only count authenticators the gateway runs on the proxy's protocol. The protocol comes from the effective `backend_scheme`: `http`/`https` proxies are HTTP-family, `tcp`/`tcps` get a TCP listener and `udp`/`dtls` a UDP listener. Every allowlisted authenticator counts on an HTTP-family proxy. On a TCP or UDP stream listener, only `mtls_auth` counts. Ferrum Edge v0.9.7 skips the other HTTP-family authenticators there, so a global `key_auth` does not protect a stream proxy. `spiffe_identity` does run on stream listeners, but it only extracts a SPIFFE ID and lets a peer without one through, so it is reported as an ignored authenticator rather than accepted. Custom allowlisted authenticators also fail closed on stream listeners, because their `supported_protocols()` contract is not visible here and Edge's default is HTTP only. A stream authenticator also needs a client certificate, so the listener must terminate TLS/DTLS (`frontend_tls: true` without `passthrough`). A stream proxy whose authenticator cannot receive a certificate is reported separately from one with no applicable authenticator. For a custom stream authenticator, either set the rule to `severity: warning` so the finding is reported without blocking apply, or use an override. An override is per PR and all-or-nothing: it releases every blocking policy finding on that PR.
...
- A credential recipient (`rotate --recipient`, or `GITFORGEOPS_ACTOR` during `apply`) must be a GitHub login: an alphanumeric first character followed by up to 38 alphanumerics or hyphens, or a `[bot]` login. Anything else is refused before SSH key discovery and before any credential is written. `apply` delivers to the same trimmed recipient its allocation journal records; a blank `GITFORGEOPS_ACTOR` means no recipient.
