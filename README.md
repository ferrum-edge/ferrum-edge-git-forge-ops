...
- `allowed_backend_domains` skips a proxy's placeholder `backend_host` only when `upstream_id` exactly resolves to a same-namespace upstream with a static target or service discovery; empty, padded, unresolved, cross-namespace, and destination-less references fall back to checking `backend_host`. The rule always checks static `targets[*].host` values and any nonblank proxy `dns_override`. Because service discovery can publish destinations that are absent from the static document, a discovery-backed upstream is reported as unverifiable unless its exact `{namespace, id}` is listed under `allowed_service_discovery_upstreams` after equivalent runtime egress controls are in place. `*.example.com` matches DNS subdomains like `api.example.com` and `deep.api.example.com`; list `example.com` separately if the root domain is allowed too. Suffix wildcards never match IP literals, which must be listed exactly. A bare `*` is an explicit catch-all for every nonempty, syntactically bare destination, including IPs.
